import { FontAwesomeIcon } from '@fortawesome/react-fontawesome'
import { faEye } from '@fortawesome/free-solid-svg-icons'
import './login.css'
import { useState } from 'react';

function Login() {
    const [toggleModal, setToggleModal] = useState(1)
    const [closeModal, setCloseModal] = useState(false)


    const LoginModal = () => {
        return (
            <>
                <div className={toggleModal === 2 ? "login-modal" : "login-modal active"}>
                    <div className="modal-top">
                        <div className="modal-top-left">
                            <img src="data:image/png;base64,"
                                alt="Logo"
                                className="modal-logo" />
                            <span className="modal-title modal-login">Login</span>
                            <span onClick={() => setToggleModal(2)} className="modal-title">Sign Up</span>
                        </div>
                        <span onClick={() => setCloseModal(true)}
                            className="modal-close">x</span>
                    </div>
                    <div className="modal-bottom">
                        <div className="modal-bottom-left">
                            <div className="modal-form">
                                <label htmlFor="phoneNumber" className="modal-label">Login via mobie</label>
                                <div className="phone-wrapper">
                                    <span className="phone-country">+84 ▼</span>
                                    <span className="input-separate"></span>
                                    <input type="tel" id="phoneNumber"
                                        placeholder='Enter Mobie Number'
                                        className="phone-input modal-input"
                                    />
                                </div>
                                <div className="password">
                                    <input type="password"
                                        placeholder='Enter Password'
                                        className="password-input modal-input"
                                    />
                                    <FontAwesomeIcon className="password-icon" icon={faEye} />
                                </div>
                                <span className="forgot-password">forgot password?</span>
                                <button className="login-button">Login</button>
                            </div>
                        </div>
                        <div className="modal-bottom-right">
                            <span className="modal-label">Log in via third-party apps</span>
                            <div className="social-link-wrapper">
                                <div className="social-icon facebook-link"></div>
                                Facebook
                            </div>
                            <div className="social-link-wrapper">
                                <div className="social-icon google-link"></div>
                                Google
                            </div>
                            <div className="social-link-wrapper">
                                <div className="social-icon twitter-link"></div>
                                Twitter
                            </div>
                            <div className="social-more">More </div>
                            <p className="terms-policy">Login means you agree the
                                <span className="hightlight-text"> Terms</span> and
                                <span className="hightlight-text"> Privacy Policy</span>
                            </p>
                        </div>
                    </div>
                </div>
            </>
        )
    }

    const SignUpModal = () => {

        return (
            <>
                < div className={toggleModal === 2 ? "signUp-modal active" : "signUp-modal"}>
                    <div className="modal-top">
                        <div className="modal-top-left">
                            <img src="data:image/png;base64,"
                                alt="Logo"
                                className="modal-logo" />
                            <span onClick={() => setToggleModal(1)} className="modal-title">Login</span>
                            <span className="modal-title modal-signup">Sign Up</span>
                        </div>
                        <span onClick={() => setCloseModal(true)}
                            className="modal-close">x</span>
                    </div>
                    <div className="modal-bottom">
                        <div className="modal-bottom-left">
                            <div className="modal-form">
                                <label htmlFor="phoneNumber" className="modal-label">Register via mobile</label>
                                <div className="phone-wrapper">
                                    <span className="phone-country">+84 ▼</span>
                                    <span className="input-separate"></span>
                                    <input type="tel" id="phoneNumber"
                                        placeholder='Enter Mobie Number'
                                        className="phone-input modal-input"
                                    />
                                </div>
                                <div className="email">
                                    <input type="text" className="email-input modal-input" placeholder='Enter verification code' />
                                    <button className='email-btn'>Receive Verificati...</button>
                                </div>
                                <div className="password">
                                    <input type="password"
                                        placeholder='Enter Password'
                                        className="password-input modal-input"
                                    />
                                    <FontAwesomeIcon className="password-icon" icon={faEye} />
                                </div>
                                <span className="password-desc">Password should be the combination of 6-20 characters and numbers</span>
                                <button className="login-button">Login</button>
                            </div>
                        </div>
                        <div className="modal-bottom-right">
                            <span className="modal-label">Log in via third-party apps</span>
                            <div className="social-link-wrapper">
                                <div className="social-icon facebook-link"></div>
                                Facebook
                            </div>
                            <div className="social-link-wrapper">
                                <div className="social-icon google-link"></div>
                                Google
                            </div>
                            <div className="social-link-wrapper">
                                <div className="social-icon twitter-link"></div>
                                Twitter
                            </div>
                            <div className="social-more">More </div>
                            <p className="terms-policy">Login means you agree the
                                <span className="hightlight-text"> Terms</span> and
                                <span className="hightlight-text"> Privacy Policy</span>
                            </p>
                        </div>
                    </div>
                </div>
            </>
        )
    }

    return (
        <div className={closeModal === true ? "login-wrapper close-login" : "login-wrapper"}>
            <div className="login-background">

                <LoginModal />
                <SignUpModal />

            </div>
        </div >
    );
}

export default Login